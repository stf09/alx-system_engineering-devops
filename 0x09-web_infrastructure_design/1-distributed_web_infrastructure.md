...
+ No monitoring.<br/>We have no way of knowing the status of each server since they are not being monitored.
+ Security issues.<br/>The data transmitted over the network is not encrypted using an SSL certificate so hackers can spy on the network. There is no way of blocking unauthorized IPs since there is no firewall installed on any server.
